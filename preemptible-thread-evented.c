...
io_thread(void *arg) {
  ...
  while (io_thread->running == 1) {
    ...
    if (io_operation->type == READ) {
      ...
      io_operation->bytes_returned = read(io_operation->fd->_fileno, &io_operation->buffer, io_operation->buffer_size);
       if (!io_operation->bytes_returned) {
...
struct lightweight_thread *lightweight_threads = 
  ...
  for (int i = 0 ; i < num_threads ; i++) {
    ...
    lightweight_threads[i].all_producers = &tinfo;
...
struct ringbuffer *io_ringbuffer = calloc(1, sizeof(*io_ringbuffer));
   io_ringbuffer->size = PENDING_IO_SIZE;
   io_ringbuffer->last_head = 0;
   io_ringbuffer->last_tail = 0;
   io_ringbuffer->tail = 0;
   io_ringbuffer->head = 0;
   struct io_operation *operations = calloc(100, sizeof(struct io_operation));
...
struct io_thread *io_thread_info = calloc(io_threads, sizeof(struct io_thread));
   ...
   s = pthread_join(timer_info->thread_id, &timer_result);
    ...
    for (int i = 0 ; i < io_threads; i++) {
        ...
        s = pthread_join(io_thread_info->thread_id, &io_thread_result);
         if (s != 0)
